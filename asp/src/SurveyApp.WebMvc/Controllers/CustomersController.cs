...
namespace SurveyApp.WebMvc.Controllers
{
    ...
    public class CustomersController : Controller
    {
        ...
        public async Task<IActionResult> Details(Guid id)
        {
            try
            {
                ...
                var viewModel = new CustomerViewModel
                {
                    ...
                    GrowthMetrics = customer.GrowthMetrics?.Select(m => new GrowthMetricViewModel
                    {
                        Period = m.Period,
                        Revenue = m.Revenue,
                        UserCount = m.UserCount
        ...
        // AJAX endpoint para obtener datos de crecimiento de un cliente específico
        [HttpGet]
        public async Task<IActionResult> GetGrowthData(Guid id)
        {
            try
            {
                var customer = await _customerService.GetCustomerByIdAsync(id);
                var growthData = customer.GrowthMetrics.Select(m => new
                {
                    period = m.Period,
                    revenue = m.Revenue,
                    userCount = m.UserCount
                }).OrderBy(m => m.period).ToList();

                return Json(growthData);
            }
            catch
            {
                return Json(new { error = "Could not retrieve growth data" });
            }
        }
    }
}
